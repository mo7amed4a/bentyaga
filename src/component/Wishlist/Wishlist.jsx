...
import { fetchAllWishlist, removeProductFromWishlist } from '../../features/wishlistSlice';
        ...
        wishlist.length > 0 && wishlist.length > 0 ? (
          <div className="mb-5">
            ...
            <div className={`pt-0 mt-5 border border-bottom-0 ${Styles.flexContainer}`}>
              <div className='w-100'>
                {wishlist.map(item => (
                  <Link to={'/productdetails/' + item.product.id} style={{textDecoration: "none", color: "#000"}} key={item.id} className={Styles.all}>
                    ...
                    <div className={`border border-2 border-black ${Styles.flexColumn}`}>
                      ...
                      <div className='d-flex justify-content-between'>
                        <h2 className={`${Styles.hh} h5 pt-4 overflow-hidden`}>{item.product.name}</h2>
                        <p className={`${Styles.hh} pt-4`}>${item.product.price}</p>
                      </div>

                      <p className={`${Styles.hh} h5 pt-4 overflow-hidden`}>{item.product.about_product	}</p>
